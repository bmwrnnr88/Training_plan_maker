...
PACE_LABELS = {
    80: "General aerobic support",
    85: "Steady / long fast support",
...
def choose_phase_lengths(
    ...
    """
    Returns a list of phase codes, one per training week.
    """
    if weeks_to_race < 6:
...
def weekly_mileage_targets(
    ...
    """
    Mileage rises gradually, then flattens or drops slightly in the specific phase.
    """
...
def hard_day_spacing(days_per_week: int, day_off: str) -> Tuple[str, str]:
    """
    Primary and secondary hard day anchors.
    """
    if days_per_week <= 4:
...
def easy_days(day_off: str) -> List[str]:
    return [d for d in DAY_ORDER if d != day_off]
