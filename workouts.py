...
def general_filler_workout(
    ...
    if event_category == "middle_distance":
        options = [
            f"10 x 1 min fast / 1 min easy",
            f"8 x 90 sec @ 105–110% / full jog",
            f"12 x 30 sec @ 110–115% / easy jog",
        ]
        ...
        options = [
            f"8 x 2 min @ 105% ({paces[105]['pace_per_mile']}/mi) / 90 sec easy",
            f"3 sets of 3-2-1 min from 100% to 108% / equal jog",
            f"20–30 min progression finishing around 90% effort",
        ]
...
def build_progression_session(
    ...
    if band == 105:
        return f"6 x 400m @ 105% / full jog"
    if band == 110:
