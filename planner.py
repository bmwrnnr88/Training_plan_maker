...
def _validate_inputs(athlete: AthleteProfile, goal: GoalProfile) -> None:
    if goal.race_distance not in EVENT_CATEGORY:
        raise ValueError(f"Unsupported target race distance: {goal.race_distance}")

    if athlete.current_distance not in EVENT_CATEGORY:
        raise ValueError(f"Unsupported current race distance: {athlete.current_distance}")

    if athlete.current_mileage > athlete.max_mileage:
        raise ValueError(
            "Current weekly mileage cannot be higher than maximum realistic weekly mileage."
        )

    if athlete.long_run_max > athlete.max_mileage:
        raise ValueError("Longest run cannot exceed maximum realistic weekly mileage.")

    if athlete.days_per_week < 3 or athlete.days_per_week > 7:
        raise ValueError("Days per week must be between 3 and 7.")

    if goal.weeks_to_race < 6:
        raise ValueError("Weeks to race must be at least 6.")
